...
const INITIAL_FORM = {
  ...
  phone: '', address: '',
  college: '', branch: '', year: '1', role: 'student',
}
...
function Input({ className = '', ...props }) {
  return (
    <input
      className={`w-full bg-gray-50 border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm text-gray-800
        placeholder:text-gray-300 focus:outline-none focus:border-teal-500 focus:bg-white
...
function Select({ children, className = '', ...props }) {
  return (
    <select
      className={`w-full bg-gray-50 border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm text-gray-800
        focus:outline-none focus:border-teal-500 focus:bg-white transition-all duration-200 ${className}`}
      {...props}
    >
      {children}
    </select>
...
/* ── Validate each step ── */
function validateStep(step, form, avatar) {
  const errors = {}
  if (step === 0) {
...
export default function Register() {
  ...
  const next = () => {
    const errs = validateStep(step, form, avatar)
  ...
  const handleSubmit = async () => {
    const errs = validateStep(2, form, avatar)
    if (Object.keys(errs).length) { setErrors(errs); return }
    ...
    try {
      // Build multipart/form-data — required because of the file upload
        ...
        <div className="flex gap-3 mt-6">
          ...
          {step < STEPS.length - 1 ? (
            ...
            <motion.button
              ...
              style={{ background: 'linear-gradient(135deg, #0d9488, #0f766e)', boxShadow: '0 4px 16px rgba(13,148,136,0.25)' }}
            >
              {loading ? <><Loader2 size={15} className="animate-spin" /> Creating account…</> : <>Create account <CheckCircle2 size={15} /></>}
            </motion.button>
          )}
        </div>
